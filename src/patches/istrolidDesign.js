Turret.prototype.rawReloadTime = undefined;
Turret.prototype.getRawReloadTime = function() { 
  if (!this.rawReloadTime) 
    return this.reloadTime;
  return this.rawReloadTime;
};
...
Turret.prototype.applyBuffs = function() {
  ...
  this.rawReloadTime = this.reloadTime;
...
cell("speed.png", ((u.maxSpeed * 16).toFixed(3)) + "m/s");
...
return div(function() {
  ...
  return div(function() {
    ...
    cell("speed.png", ((u.maxSpeed * 16).toFixed(2)) + "m/s", "max unit move speed");
    if (u.jumpDistance) {
      cell("jump.png", (u.jumpDistance.toFixed(1)) + "m", "jump distance (" + u.mass * 15 +"e min.)");
    }
    ...
    cell("arc360.png", (u.radius.toFixed(6)) + "m", "ship size");
    
    if(u.fireEnergy > 0)
    {
        divider();
        cell("letters.png", (u.weaponDPS / u.fireEnergy ).toFixed(4) + " dee", ": dps energy efficiency");
        cell("ai.png", (u.weaponDPS / u.cost ).toFixed(4) + " dce", ": dps cost efficiency");
        cell("damage.png",  (u.weapons.map(x => x.damage).reduce((a,b) => a + b, 0)).toFixed(1) + " dmg", ": burst dmg");
        cell("mute.png",  (u.hp/ u.cost).toFixed(4) + " hce", ": hp cost efficiency");
...
cell("reload.png", ((w.reloadTime / 16).toFixed(2)) + "s", "reload time sec");
cell("reload.png", (w.getRawReloadTime().toFixed(2)) + " tix", "reload time tick");
...
if ((ref1 = w.bulletCls) != null ? ref1.prototype.missile : void 0) {
  cell("missle.png", "Missile", "missile based, can be shot down by point defence");
}
if (w.hitsMissiles) {
...
return div("#hoverInfo", function() {
  ...
  if (part.genShield) {
    ...
    cell("shield.png", "+" + (part.useEnergy / part.genShield).toFixed(1) + "e/sh", "recharges shield");
  }
  if (part.shield) {
  ...
  if (part.slow) {
    cell("speed.png", "-" + part.slow * 16 + "m/s", "Slows down ships in range");
  ...
  if (part.rangeBuffMul) {
    return cell("range.png", pr(-100 + (part.rangeBuffMul * 100)), "range");
  }
});
};
